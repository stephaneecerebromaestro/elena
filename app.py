...
@app.route("/aria/test/webhook-debug", methods=["POST"])
def aria_test_webhook_debug():
    """
    Endpoint de diagnóstico que procesa un callback de Telegram
    y devuelve el resultado interno (en lugar de siempre {"ok":true}).
    """
    ...
    try:
        data = request.get_json(force=True)
        debug_info["raw_data_keys"] = list(data.keys()) if data else None
        debug_info["has_callback_query"] = "callback_query" in (data or {})
        
        if not data:
            return jsonify({"error": "no_data", "debug": debug_info})
        
        callback_query = data.get("callback_query")
        if not callback_query:
            return jsonify({"error": "no_callback_query", "debug": debug_info})
        
        callback_data = callback_query.get("data", "")
        debug_info["callback_data"] = callback_data
        debug_info["has_colon"] = ":" in callback_data
        
        if ":" not in callback_data:
            return jsonify({"error": "invalid_format", "debug": debug_info})
        
        action, correction_id = callback_data.split(":", 1)
        approved = (action == "approve")
        debug_info["action"] = action
        debug_info["correction_id"] = correction_id
        debug_info["approved"] = approved
        
        # Ejecutar apply_correction
        from aria_audit import apply_correction
        result = apply_correction(correction_id, approved)
        debug_info["apply_correction_result"] = result
        
        return jsonify({"status": "ok", "debug": debug_info})
        
    except Exception as e:
        debug_info["exception"] = f"{type(e).__name__}: {str(e)}"
        debug_info["traceback"] = traceback.format_exc()
        return jsonify({"status": "error", "debug": debug_info})
